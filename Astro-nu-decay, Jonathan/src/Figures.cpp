...
void Visible_m1()
{
	...
	for (int i = 0; i <= n; i++)
	{
		...
		// We change the scale, and then loop over each new component: 
		dp.Ef = Efmin * pow(Efscale, i);
		psm = PSM(a, b, dp);
...
void Visible_gamma()
{
	...
	for (int i = 0; i <= n; i++)
	{
		...
		for (int j = 0; j < n_gamma; j++)
		{
			...
			// Overvej lige at gøre det på denne måde i stedet: 
			dp.gamma = gammas[j];
...
{
	printf("Rtc\n");
	// Grundparameterene defineres igen: 
