...
/*  This function has been modified by changing the integration technique from QAG to Romberg or Fixed Quadrature integration. 
	 This has been done to speed up the process. The original lines are commented out.  
...
double RSE_integral(double zmax, bool ykbh)
{
   ...
   int n = 50;
    ...
    gsl_integration_fixed_workspace* w 
       = gsl_integration_fixed_alloc(T, n, 0, zmax, 0.0, 0.0);
    gsl_integration_fixed(&F, &integral, w);
...
double I2_calc(double zi, double zf)
{
   ...
   // New Romberg Lines:
    ...
    gsl_integration_romberg_workspace* w 
        = gsl_integration_romberg_alloc(ROMB_N);
    gsl_integration_romberg(&F, zi, zf, EPSABS, EPSREL, &integral, &neval, w);
